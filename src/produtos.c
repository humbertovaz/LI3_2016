...
/*A funcão inicializa_catalogo_produtos aloca memória para o respetivo catalogo de todos os produtos, e cria 27 AVLs para cada letra dos produtos através da funcão avl_create*/
...
/*A funcão free_catalogo_produtos destrói os dados do catalogo de produtos (cat) um a um (fazendo libavl_free) e no final liberta a memória do catálogo */
...
/*A funcão listaProdutosLetra, dado uma letra e um catalogo em parâmetro, através de um traverser associada à estrutura avl dessa letra, vai inserindo num array a, todos os ids de produtos começados por essa letra. Fazendo o respetivo free do traverser*/
...
/*A funcão existeProduto verifica se existe um produto (através do seu id) num catálogo (ambos passados por parâmetro) se existe devolve 1 se não 0*/
int existeProduto (char *produto,CatProdutos cat) {
    if (avl_find(cat->indices[produto[0]-'A'], produto) != NULL) return 1;
    else return 0;
}
/*A funcão numeroProdutos faz a soma de todos os nodos de cada avl, uma a uma. Ou seja através de um ciclo for e da funcão avl_count iremos obter a soma total de produtos das 27 avl's */
...
/*A funcão numeroProdutosLetra com o auxilio da funcão avl_count, dado um catalogo e letra ambos passados por parâmetro, calcula o nr de nodos da avl associada a essa letra*/
